...
nodefony.registerCommand("MochaUnit", function(){
	...
	var MochaUnit = function(container, command, options){
		...
		this.kernel.listen(this, 'onReady', function(kernel){
			...
			switch ( arg[1] ){
				...
				default:
					this.logger(new Error("MochaUnit   "+command[1] +" bad format"), "ERROR");
					this.showHelp();
					this.terminate(1);
	...
	return {
		name: "MochaUnit",
		commands: {
			listAll: ["MochaUnit:list:all", "List all unit tests"],
			listBundle: ["MochaUnit:list:bundle bundleName", "List all bundle unit tests"],
			launchAll: ["MochaUnit:launch:all", "Launch all tests"],
			launchBundle: ["MochaUnit:launch:bundle bundleName", "Launch bundle tests"],
			launchSingle: ["MochaUnit:launch:single bundleName testfile", "Launch single test"]

...
stage.register('CCAPI', function(){
	

	var CCAPITranport = function(url, settings){
		return  new stage.io.transports.websocket(url, settings);
		
	}

	var CCAPI = function(kernel, settings){
		this.kernel = kernel ;
	...
	CCAPI.prototype.listen = function(){
		return this.notificationsCenter.listen.apply(this.notificationsCenter, arguments);
	};

	CCAPI.prototype.fire = function(){
		return this.notificationsCenter.fire.apply(this.notificationsCenter, arguments);
	};

	CCAPI.prototype.info = function(message , sev){
		return this.kernel.info(message , sev);
	};

	CCAPI.prototype.close = function(){
		this.unRegister();
		//this.transport.close() ;
		delete this.transport ;
	};

	CCAPI.prototype.connect = function(){
		if (this.transport ){
			this.close() ;	
		}
		this.transport = CCAPITranport(this.settings.WebSockServerUrl, {
			//websocket protocol 
			protocol:"sip"
		});
	...
	CCAPI.prototype.reConnect = function(){
		delete this.webrtc ;
		this.connect();
	}

	CCAPI.prototype.register = function(user, passwd){
		if ( this.webrtc ){
	...
	CCAPI.prototype.unRegister = function(){
		if ( this.webrtc ){
			this.webrtc.unRegister();
		}
	};

	CCAPI.prototype.initWebrtc = function(transport){
		this.webrtc = null ;

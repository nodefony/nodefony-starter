...
_BASE_REGEX_STRING = '^\s*nodefony\.%s\(\s*[\'"](.+)[\'"]\s*\)'
...
def CalculateDependencies(paths, inputs):
  ...
  if base_js_path:
    ...
    logging.warning('nodefony Library nodefony.js not found.')
...
def GetDepsLine(dep, base_path):
  ...
  """
  return 'nodefony.addDependency("%s", %s, %s);' % (
...
def Compile(compiler_jar_path, source_paths, out, flags=None):
  ...
  """YAHOO COMPILER
  """
  if compiler_jar_path == "./tools/yahoo/yuicompressor-2.4.2.jar" :
    for path in source_paths:
      args = ['java', '-jar', compiler_jar_path]
      if flags:
        args += flags
      if path != 'tmp/require.js':
        args += [path]
        ...
        if proc.returncode != 0:
          ...
          logging.info('Compiling with the following error: %s', stdoutdata)
          sys.exit(1)
        else:
...
def main():
  ...
  if options.output_mode == 'deps':
    ...
    if not result:
      logging.error('Could not find nodefony Library in the specified paths')
      sys.exit(1)
  ...
  logging.info('Finding nodefony dependencies...')
